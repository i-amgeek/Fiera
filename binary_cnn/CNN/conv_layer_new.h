...
struct conv_layer_t
{
    ...
    {   
        this->filters = eval(xt::random::rand({number_filters, extend_filter, extend_filter, in.size.z}));
    ...
    tensor_4d activate(tensor_4d& in, bool train)
    {
        ...
        int HH = weights.shape()[2];
        int WW = weights.shape()[3];
        int H_prime = (H-HH) / stride + 1
        int W_prime = (W-WW) / stride + 1

        tensor_3d in_col = im2col(in, weights.shape(), stride);
        tensor_2d filters_col = filters.reshape({F, -1});
