...
struct batch_norm_layer_t
{
	...
	tensor_4d activate(tensor_4d& in, bool train = true){
		...
		if(train) {
			...
			tensor_1d sigma = xt::mean(mean_diff * mean_diff, {0, 2, 3});
	...
	tensor_4d calc_grads( tensor_4d& grad_next_layer)
	{
		...
		tensor_1d grads_sqrtvar = -1.0 * xt::sum(grads_in_hat * mean_diff, {0,2,3}) / (sqrt_sigma * sqrt_sigma);
		...
		tensor_4d grads_xmul2 = make1Dto4D(grads_var) * 2 * mean_diff / (out_size.m * out_size.x * out_size.y);
