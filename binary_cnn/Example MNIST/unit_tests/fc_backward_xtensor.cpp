...
// std::copy(temp_in.shape().cbegin(), temp_in.shape().cend(), std::ostream_iterator<size_t>(std::cout, " ")); // To print shape of xtensor
    ...
    xarray<float> grads_next_layer = {
        {0.0848, -0.3588,  0.1876,  0.0864},
        {-0.3660,  0.1304,  0.1203,  0.1153}};

    xarray<float> expected_grads_in = {{-0.052139,  0.031144, -0.031169,  0.059484, -0.096869, -0.076348,
   0.015489, -0.073215},
 {-0.087639,  0.187585, -0.046781, -0.032198, -0.051517, -0.065747,
   0.046043, -0.076833}};
    ...
    xarray<float> grads_in = layer->calc_grads(grads_next_layer);

    layer->fix_weights(0.01);

    xarray<float> expected_updated_weights{{ 0.329617, -0.279922,  0.270211, -0.062576,  0.181786,  0.138135,
   0.014739,  0.066529},
